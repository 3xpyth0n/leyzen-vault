...
func RenderHAProxyConfig(
	...
	sb.WriteString("    # Don't intercept 404 for API routes (let Flask handle JSON responses)\n")
	sb.WriteString("    http-request set-var(txn.is_api) bool(true) if { path_beg /api }\n")
	sb.WriteString("    http-response return status 404 errorfile /usr/local/etc/haproxy/errors/404.http if { status 404 } !{ var(txn.is_api) -m bool }\n")
