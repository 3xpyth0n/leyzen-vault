...
- The orchestrator is designed to run behind a trusted reverse proxy. Requests
  ...
  forwarding headers.【F:orchestrator/__init__.py†L25-L39】【F:orchestrator/config.py†L64-L81】
- Environment variables provide credentials, secrets, and the allowlist of
  ...
  when required values are missing or malformed.【F:orchestrator/config.py†L43-L60】【F:orchestrator/config.py†L82-L142】
- Application logs are written to a dedicated file whose location can be
  ...
  file permissions can be managed by deployment tooling.【F:orchestrator/config.py†L101-L142】
...
- Administrator authentication relies on `werkzeug.security` password hashes
  ...
  stored.【F:orchestrator/config.py†L122-L138】
- The login form issues rotating CSRF tokens kept in an in-memory store with a
  ...
  POSTs, and are retired immediately after use.【F:orchestrator/blueprints/auth.py†L231-L276】【F:orchestrator/blueprints/auth.py†L302-L360】
- Human verification is enforced through a captcha generator that supports both
  ...
  server-side and invalidated after verification.【F:orchestrator/blueprints/auth.py†L62-L217】【F:orchestrator/blueprints/auth.py†L278-L347】
- Repeated failures trigger IP-based rate limiting with a five minute backoff
  window, blocking automated guessing attempts.【F:orchestrator/blueprints/auth.py†L196-L230】【F:orchestrator/blueprints/auth.py†L321-L360】
- Sessions are flagged `HttpOnly` and `SameSite=Lax`; background workers are
  started lazily to limit exposure before the first authenticated request.【F:orchestrator/__init__.py†L30-L57】【F:orchestrator/app.py†L39-L63】
...
- A strict Content Security Policy is injected on every response, forbidding
  ...
  routing violation reports to a dedicated endpoint.【F:orchestrator/__init__.py†L45-L63】
- The CSP report collector enforces size limits and per-IP rate caps before
  ...
  flooding attacks.【F:orchestrator/blueprints/dashboard.py†L104-L190】
- Static asset routes apply explicit MIME types and cache-control headers while
  keeping the dashboard JS bundle authenticated-only.【F:orchestrator/blueprints/dashboard.py†L80-L135】
- Client IP detection respects `X-Forwarded-For` only for routable addresses,
  ...
  loopback ranges.【F:orchestrator/blueprints/utils.py†L11-L40】
...
- The orchestrator delegates container lifecycle operations to a
  ...
  rejected server-side.【F:orchestrator/services/docker_proxy.py†L15-L118】【F:orchestrator/services/docker_proxy.py†L170-L218】
- Cached `docker inspect` payloads are bounded by a short TTL to avoid leaking
  stale state to operators while still minimizing proxy traffic.【F:orchestrator/services/docker_proxy.py†L126-L218】
- Control endpoints validate state transitions (e.g., disallowing kill when the
  ...
  client IP.【F:orchestrator/blueprints/dashboard.py†L31-L103】

## Logging and observability

- All structured log entries are sanitized to remove control characters before
  writing to disk, mitigating log-forging attacks.【F:orchestrator/services/logging.py†L1-L48】
- Authentication, control requests, and CSP reports include contextual metadata
  to help correlate suspicious activity while avoiding sensitive payloads.【F:orchestrator/blueprints/auth.py†L314-L360】【F:orchestrator/blueprints/dashboard.py†L31-L190】
...
- Session cookies are not yet marked `Secure`; deployments behind TLS-terminating
  ...
  is guaranteed end-to-end.【F:orchestrator/__init__.py†L30-L45】
