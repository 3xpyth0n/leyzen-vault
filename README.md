...
- Un reverse proxy (HAProxy) exposant un point unique vers le seul serveur actif.
...
2. Démarrer l'orchestrateur pour activer la rotation `python3 vault_orchestrator.py`.
3. Vérifier les headers HTTP via `curl -I http://localhost:8080` pour observer la rotation.
