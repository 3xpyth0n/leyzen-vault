...
class RotationTelemetry:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._metrics_history: deque[dict[str, object]] = deque(
            maxlen=self._calculate_history_maxlen(
                ...
                metrics_interval,
                cap=TELEMETRY_METRICS_CAP,
            )
        )
        self._metrics_history_lock = Lock()

        self._previous_net_snapshot: dict[str, float] | None = None
    ...
    def build_metrics(
        ...
        stats_lookup: Callable[[str], dict[str, object] | None],
        ...
        net_payload: dict[str, float] | None = None
    ...
    # ------------------------------------------------------------------
    def append_metrics_history(self, metrics: dict[str, object] | None) -> None:
        if not metrics:
            return
    ...
    # ------------------------------------------------------------------
    def store_latest_metrics(self, metrics: dict[str, object] | None) -> None:
        with self._metrics_lock:
            self._latest_metrics = metrics
            self._metrics_updated_at = time.time()

    def get_cached_metrics(self) -> tuple[dict[str, object] | None, float]:
        with self._metrics_lock:
    ...
    def get_snapshot_cache(self) -> tuple[dict[str, object] | None, float]:
        with self._snapshot_lock:
    ...
    # ------------------------------------------------------------------
    def _compute_cpu_percent(self, stats: dict[str, object]) -> float | None:
    ...
    def _calculate_history_maxlen(
        ...
        interval = max(TELEMETRY_HISTORY_MIN_INTERVAL, float(interval_seconds))
        estimate = int(math.ceil(window_seconds / interval))
        return max(1, min(estimate + TELEMETRY_HISTORY_BUFFER, cap))
