...
class DockerProxyClient:
    ...
    def __init__(
        ...
        self.base_url = (base_url or DOCKER_PROXY_BASE_URL_DEFAULT).rstrip("/")
        self.token = token
        self.timeout = (
            timeout if timeout is not None else DOCKER_PROXY_CLIENT_TIMEOUT_DEFAULT
        )
...
class ProxyContainer:
    ...
    def __init__(
        ...
        *,
        cache_update: Callable[[dict[str, Any]], None] | None = None,
        cache_invalidate: Callable[[], None] | None = None,
...
INSPECT_CACHE_TTL = DOCKER_PROXY_INSPECT_CACHE_TTL
...
class DockerProxyService:
    ...
    def _get_cached_payload(self, name: str) -> tuple[dict[str, Any], float] | None:
    ...
    def _log_proxy_error(self, name: str, exc: Exception) -> None:
        ...
        if "Failed to reach docker proxy" in message:
            if self._should_log_proxy_error(
                "connection", interval=DOCKER_PROXY_ERROR_LOG_INTERVAL
            ):
                self._logger.log(
    ...
    def wait_until_healthy(
        ...
        *,
        ...
        if check_interval is None:
            check_interval = DOCKER_PROXY_HEALTH_CHECK_INTERVAL
        if log_interval is None:
            log_interval = DOCKER_PROXY_HEALTH_LOG_INTERVAL
        last_log = time.time()
    ...
    def stop_container(
        self, name: str, *, reason: str | None = None, timeout: int = 30
    ) -> bool:
